    ...
    $.ajax({
        ...
        success: function (data) { // success est toujours en place, bien sûr !
            $(data.results).each(function () {
                ...
                $('<span></span>').appendTo('#listeamis').html(prenom + ' ' + nom).addClass(sexe).toggleClass("toto col-xs-6 hidden-sm hidden-xs");
                $('<img>').appendTo('#listeamis').attr('src', this.picture.large).addClass("tof col-md-6 col-xs-12");
